...
This is part of a larger, separately-planned effort (see `plan-gitHubPageClient.md`): Phase 0 (build-step refactor), Phase 1 (unified YAML format, breaking, `APP_VERSION` 2.0.0), Phase 2 (hosted-client build artifact, `APP_VERSION` 2.1.0), and Phase 3 (Pages deploy workflow + manager UI + docs, `APP_VERSION` 2.2.0) are all done. `.github/workflows/pages.yml` builds and publishes `client/` to GitHub Pages on push to `master` (plus `workflow_dispatch`); the canonical hosted-client URL is `https://camaro1.github.io/UmfrageManager/`. **One remaining manual step is outside any workflow's reach:** the repository owner must go to *Settings → Pages* once and set Source to "GitHub Actions" and enable "Enforce HTTPS" — until that's done, the workflow builds and validates correctly but nothing is actually served. Don't assume that manual step has happened; treat it as a standing precondition, not something this repo's automation can confirm.

A full formal threat-model update for the hosted client (STRIDE re-analysis, risk scoring) is a **separate, not-yet-done** follow-up effort (`/initPlan threatModelUpdate`) — `docs/security/threat-model.md` does not yet reflect this feature beyond the README-level summary added in Phase 3. Don't assume the formal threat model already covers it.
...
- **Deploy (Phase 3, done):** `.github/workflows/pages.yml` runs `python3 build.py` on push to `master` (paths: `src/**`, `build.py`, `client/**`, `UmfrageManager.html`, the workflow file itself) and on `workflow_dispatch`, fails the job via `git diff --exit-code -- UmfrageManager.html client/index.html` if the committed build artifacts don't match what `build.py` regenerates from `src/` (the freshness check), then publishes `client/` via `actions/configure-pages` + `actions/upload-pages-artifact` + `actions/deploy-pages`. The canonical URL, `CANONICAL_CLIENT_URL = 'https://camaro1.github.io/UmfrageManager/'`, lives as a single constant in `src/manager/manager.js` (alongside `GETTING_STARTED_URL`, the absolute `https://github.com/...` link to `docs/getting-started.md`) — referenced, not duplicated, everywhere the manager displays it (the post-export info modal shown after "Als YAML exportieren", and the manager's own help box behind the header's `?` button, analogous to the client's existing help-box pattern). Flipping the actual Pages source setting and enabling "Enforce HTTPS" in the repo's Settings is a one-time manual step for the repository owner — no workflow or code change can do that part.
